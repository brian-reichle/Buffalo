...
namespace Buffalo.Core.Parser.Configuration.Test
{
	...
	{
		[Test]
		public void ParserName()
		{
			var manager = new ConfigManager();
			manager.Reset();

			Assert.That(manager.ParserName, Is.EqualTo("Parser"));

			var labelToken = new Mock<IToken>();
			var valueToken = new Mock<IToken>();
			...
			Assert.That(manager.ParserName, Is.EqualTo("NewName"));
		}
		...
		public void TypeName()
		{
			...
			Assert.That(manager.TypeName, Is.EqualTo("ParserTokenType"));
			...
			labelToken.Setup(x => x.Text).Returns("TypeName");
			valueToken.Setup(x => x.Text).Returns("NewName");
			valueToken.Setup(x => x.Type).Returns(SettingTokenType.String);

			manager.Set(reporter.Object, labelToken.Object, valueToken.Object);

			Assert.That(manager.TypeName, Is.EqualTo("NewName"));
		}
		...
		[Test]
		public void RenderParseTable()
		{
			var manager = new ConfigManager();
			manager.Reset();

			Assert.That(manager.RenderParseTable, Is.EqualTo(false));

			var labelToken = new Mock<IToken>(MockBehavior.Strict);
			var valueToken = new Mock<IToken>(MockBehavior.Strict);
			var reporter = new Mock<IErrorReporter>(MockBehavior.Strict);

			labelToken.Setup(x => x.Text).Returns("RenderParseTable");
			valueToken.Setup(x => x.Text).Returns("true");
			valueToken.Setup(x => x.Type).Returns(SettingTokenType.Label);

			manager.Set(reporter.Object, labelToken.Object, valueToken.Object);

			Assert.That(manager.RenderParseTable, Is.EqualTo(true));
		}
