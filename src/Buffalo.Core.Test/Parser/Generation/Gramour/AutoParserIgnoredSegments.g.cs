...
namespace Buffalo.Generated.IgnoredSegments
{
	...
	internal abstract class AutoParser
	{
		...
		//   With SR Conflicts : 0
		//   Other             : 2
		// Transition Table    : 15/24(62.50%)
		//   Primary Offsets   : 6
		//   Goto Offsets      : 1
		//   Actions           : 8
		// Memory Footprint    : 30 bytes
		// Assembly Footprint  : 30 bytes (100.00%)
			...
			using (IEnumerator<Type_Token> enumerator = tokens.GetEnumerator())
			{
				...
				while (haveToken)
				{
...

					{
						...
						int action = offset <= 3 ? offset : _transitionTable[offset + (int)type];

						if (action > 3)
						{
							State newState = new State();
							newState.state = state = action - 3;
		...
		int Reduce(int reductionId, List<State> stack)
		{
			...
			{
				case 0: // <Document> -> A <A>
					newState.value = Reduce_Document_1((Type_Token)stack[stack.Count - 2].value);
					gotoIndex = 0;
					stack.RemoveRange(stack.Count - 2, 2);
					break;

				case 1: // <Document> -> B B
					newState.value = Reduce_Document_2((Type_Token)stack[stack.Count - 2].value);
					gotoIndex = 0;
					stack.RemoveRange(stack.Count - 2, 2);
					break;
		...
		protected abstract Type_Value Reduce_Document_1(Type_Token aSeg);
		...
		{
			return Extract("Buffalo.Core.Test.Parser.Generation.Gramour.AutoParserIgnoredSegments.table");
		}
	}
}
