...
namespace Buffalo.Generated.Field
{
	...
	internal abstract class AutoParser
	{
		...
		//   Goto Offsets      : 11
		//   Actions           : 113
		// Memory Footprint    : 304 bytes
		// Assembly Footprint  : 256 bytes (84.21%)
		...
		{
			return Parse(tokens, 0).value_Object;
		}
		...
		int Reduce(int reductionId, List<State> stack)
		{
			...
			switch (reductionId)
			{
				case 14: // <Equation> -> <Expression> <ComparisonOp> <Expression>
					newState.value_Object = Reduce_Equation_1(stack[stack.Count - 3].value_Object, stack[stack.Count - 2].value_Object, stack[stack.Count - 1].value_Object);
					gotoIndex = 4;
					stack.RemoveRange(stack.Count - 3, 3);
					break;

				case 4: // <ComparisonOp> -> EqualTo
					newState.value_Object = Reduce_ComparisonOp_1(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 9: // <ComparisonOp> -> NotEqualTo
					newState.value_Object = Reduce_ComparisonOp_2(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 7: // <ComparisonOp> -> LessThan
					newState.value_Object = Reduce_ComparisonOp_3(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 8: // <ComparisonOp> -> LessThanOrEqualTo
					newState.value_Object = Reduce_ComparisonOp_4(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 5: // <ComparisonOp> -> GreaterThan
					newState.value_Object = Reduce_ComparisonOp_5(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 6: // <ComparisonOp> -> GreaterThanOrEqualTo
					newState.value_Object = Reduce_ComparisonOp_6(stack[stack.Count - 1].value_Token);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 15: // <Expression> -> <Expression> <TermOp> <Term>
					newState.value_Object = Reduce_Expression_1(stack[stack.Count - 3].value_Object, stack[stack.Count - 2].value_Object, stack[stack.Count - 1].value_Object);
					gotoIndex = 3;
					stack.RemoveRange(stack.Count - 3, 3);
					break;

				case 0: // <Expression> -> <Term>
					newState.value_Object = Reduce_Expression_2(stack[stack.Count - 1].value_Object);
					gotoIndex = 3;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 10: // <TermOp> -> Add
					newState.value_Object = Reduce_TermOp_1(stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 11: // <TermOp> -> Subtract
					newState.value_Object = Reduce_TermOp_2(stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 16: // <Term> -> <Term> <FactorOp> <Factor>
					newState.value_Object = Reduce_Term_1(stack[stack.Count - 3].value_Object, stack[stack.Count - 2].value_Object, stack[stack.Count - 1].value_Object);
					gotoIndex = 2;
					stack.RemoveRange(stack.Count - 3, 3);
					break;

				case 2: // <Term> -> <Factor>
					newState.value_Object = Reduce_Term_2(stack[stack.Count - 1].value_Object);
					gotoIndex = 2;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 13: // <FactorOp> -> Multiply
					newState.value_Object = Reduce_FactorOp_1(stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 12: // <FactorOp> -> Divide
					newState.value_Object = Reduce_FactorOp_2(stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 17: // <Factor> -> <Value> Power <Value>
					newState.value_Object = Reduce_Factor_1(stack[stack.Count - 3].value_Object, stack[stack.Count - 2].value_Token, stack[stack.Count - 1].value_Object);
					gotoIndex = 1;
					stack.RemoveRange(stack.Count - 3, 3);
					break;

				case 1: // <Factor> -> <Value>
					newState.value_Object = Reduce_Factor_2(stack[stack.Count - 1].value_Object);
					gotoIndex = 1;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 3: // <Value> -> Number
					newState.value_Object = Reduce_Value_1(stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveAt(stack.Count - 1);
					break;

				case 18: // <Value> -> OpenParen <Expression> CloseParen
					newState.value_Object = Reduce_Value_2(stack[stack.Count - 3].value_Token, stack[stack.Count - 2].value_Object, stack[stack.Count - 1].value_Token);
					gotoIndex = 0;
					stack.RemoveRange(stack.Count - 3, 3);
					break;
		...
		{
			public int state;
			public Type_Token value_Token;
			public Type_Object value_Object;
		}

		static ushort[] GetTransitionTable()
		{
			return Expand("Buffalo.Core.Test.Parser.Generation.TypeHandling.AutoParserField.table");
		}
	}
}
